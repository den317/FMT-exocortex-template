---
name: wp-new
description: Создание нового рабочего продукта (РП) с атомарной записью в 4 локальных места и GitHub issue в owner-repository; Linear получает задачу через GitHub. Используй когда появляется новая задача, которой нет в плане недели.
...
Создание нового рабочего продукта (РП) с атомарной записью в 4 локальных места и GitHub issue в owner-repository; Linear получает задачу через GitHub. Используй когда появляется новая задача, которой нет в плане недели.

## ⛔ БЛОКИРУЮЩЕЕ ПРАВИЛО (Шаг 0): явное согласие пользователя

**Создание РП — необратимое действие** (запись в 4 локальных места: REGISTRY/WeekPlan/active-wp/context-file; GitHub issue — внешний пост-шаг WP-34). Откат стоит времени и создаёт шум в системе. Поэтому:
...
- внешний отказ не откатывает локальную регистрацию РП.
...
Выведи: *«РП #{N} создан. Скрипт записал в 4 местах: context file, Registry, WeekPlan, active-wp пересобран. GitHub issue — {URL}; Linear — штатная GitHub-интеграция.»*
