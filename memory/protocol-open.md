...
5a. **Пост-шаг: GitHub Issue → Linear (WP-34).** ПОСЛЕ полной локальной записи создать или найти ровно одну GitHub issue в owner-repository. Linear получает связанную задачу штатной GitHub-интеграцией; прямой Linear MCP/API не вызывать. Недоступность GitHub не откатывает локальный РП: отметить внешний шаг незавершённым.
